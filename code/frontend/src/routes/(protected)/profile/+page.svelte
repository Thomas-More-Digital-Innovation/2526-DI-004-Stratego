<script lang="ts">
	...
	function getWinRate(): string {
		...
		return ((data.stats.wins / data.stats.total_games) * 100).toFixed(1) + '%';
	}
</script>
	...
	<div class="profile-card">
		<div class="profile-header">
			{#if data.user.profile_picture}
				<img src={data.user.profile_picture} alt="Profile" class="profile-pic" />
			{:else}
			...
			<div class="stats-grid">
				...
				<div class="stat-card">
					<div class="stat-value">
						{formatDuration(data.stats.avg_game_duration_seconds)}
					</div>
		...
		<div class="actions">
			<a href="/board-setups" class="action-btn">Manage Board Setups</a>
			<a href="/" class="action-btn secondary">Play Game</a>
		</div>
	</div>
</div>
...
<style>
	...
	.profile-pic-placeholder {
		...
		height: 100px;
		border-radius: 50%;
		background: linear-gradient(135deg, #66f, #44d);
		color: white;
		display: grid;
		place-items: center;
		font-size: 2.5rem;
