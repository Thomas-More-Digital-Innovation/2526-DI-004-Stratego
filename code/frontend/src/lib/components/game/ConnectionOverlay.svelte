...
>
    {#if isReconnecting}
        <div class="relative flex items-center justify-center w-20 h-20">
            <div
                class="absolute inset-0 rounded-full border-4 border-brand-accent/20 border-t-brand-accent animate-spin"
            ></div>
            <div class="text-brand-accent text-3xl animate-pulse">⚡</div>
        </div>
        <div class="space-y-2">
            <h2 class="text-2xl font-black text-white tracking-wider uppercase">
                Connection Lost
            </h2>
    ...
    {:else if !serverStore.isOnline}
        <div
            class="flex items-center justify-center w-16 h-16 rounded-2xl bg-brand-secondary/10 border border-brand-secondary/20 text-brand-secondary text-3xl"
        >
            🔌
        </div>
        <div class="space-y-2">
    ...
    {:else}
        <div
            class="flex items-center justify-center w-16 h-16 rounded-2xl bg-brand-accent/10 border border-brand-accent/20 text-brand-accent text-3xl"
        >
            🔍
        </div>
        <div class="space-y-2">
            ...
            <p class="text-white/60 text-sm">
                The game session <span class="font-mono text-brand-accent font-semibold">{gameId}</span> could not be found. It may have expired, been cleaned up, or never existed.
            </p>
        </div>
        <div class="flex flex-col sm:flex-row gap-3 w-full mt-4">
            <Button variant="secondary" class="w-full" onclick={onReturnToMenu}>
