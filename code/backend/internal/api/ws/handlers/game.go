...
func handleMove(c *ws.Client, data json.RawMessage) error {
	...
	from := game.NewPosition(moveMsg.From.X, moveMsg.From.Y)
	to := game.NewPosition(moveMsg.To.X, moveMsg.To.Y)
...
func handleGetValidMoves(c *ws.Client, data json.RawMessage) error {
	...
	pos := game.NewPosition(reqMsg.Position.X, reqMsg.Position.Y)
