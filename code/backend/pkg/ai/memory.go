...
func (m *Memory) Remember(pos game.Position, piece *game.Piece, confidence float64, round int) {
	m.mutex.Lock()
...
func (m *Memory) Recall(pos game.Position) *MemoryEntry {
	m.mutex.RLock()
...
func (m *Memory) Forget(pos game.Position) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
func (m *Memory) MovePiece(from, to game.Position) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
func (m *Memory) UpdateFromCombat(attackerPos, defenderPos game.Position, attackerPiece, defenderPiece *game.Piece, round int) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
func (m *Memory) GetKnownEnemyPositions() []game.Position {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
