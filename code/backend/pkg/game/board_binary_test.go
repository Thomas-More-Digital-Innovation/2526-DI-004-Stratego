...
func TestValidateSetup(t *testing.T) {
	...
	// Wrong row count
	if err := ValidateSetup([]string{rowStr}); err == nil {
		t.Error("Wrong row count should be rejected")
	}

	// Wrong row length
	if err := ValidateSetup([]string{rowStr, rowStr, rowStr, rowStr}); err == nil {
...
func TestEncodeSetupPlayer2(t *testing.T) {
	...
	if data[0]&BitColor == 0 {
		t.Error("Player 2 piece should have BitColor set")
	}

	decodedRows, playerID, _ := DecodeSetup(data)
	if playerID != 2 {
