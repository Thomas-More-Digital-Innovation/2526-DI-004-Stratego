...
func runServer(addr string) {
	...
	fmt.Println("  GET  /users - Get user by ID")
	fmt.Println("  GET  /users/stats - Get user statistics")
	fmt.Println("  GET  /board-setups - Get board setups for user")
	fmt.Println("  POST /board-setups - Create board setup")
	fmt.Println("  PUT  /board-setups - Update board setup")
	fmt.Println("  DELETE /board-setups - Delete board setup")
	fmt.Println("  POST /games - Create a new game")
	fmt.Println("  GET  /games - List all games")
	fmt.Println("  WS   /game/{gameID}?player={0|1|spectator} - Connect to game")
