...
func TestHub_Broadcasters(t *testing.T) {
	...
	combat := &game.CombatResult{
		Occurred:         true,
		AttackerPiece:    engine.NewPiece(models.Marshal, &player1),
		DefenderPiece:    engine.NewPiece(models.General, &player2),
		AttackerPosition: engine.NewPosition(0, 0),
		DefenderPosition: engine.NewPosition(0, 1),
	}
...
func TestHub_Perspectives(_ *testing.T) {
	...
	board.SetPieceAt(engine.NewPosition(5, 5), engine.NewPiece(models.Marshal, &player1))
	board.SetPieceAt(engine.NewPosition(5, 6), engine.NewPiece(models.General, &player2))
